...
// Recupera o ID do professor da URL
...
// Verifica se há arquivos associados ao professor
if (!empty($professor['arquivo'])) {
    // Converte a string de arquivos em um array
    $arquivos = explode(",", $professor['arquivo']);
} else {
...
<body>
    ...
    <div class="container">
        <h1 class="mt-4">Dados do Professor</h1>
        <div class="card">
            <div class="card-body">
                ...
                <p class="card-text"><i class="fas fa-birthday-cake mr-2"></i><strong>Data de Nascimento:</strong>
                    <?php echo date('d/m/Y', strtotime($professor['data_nascimento'])); ?>
                </p>
        ...
        <form id="cadastro-form" enctype="multipart/form-data">
            <div class="input-group">
                <input type="hidden" name="id" value="<?= $id_professor ?>">
        ...
        <ul class="list-group mt-3" id="lista-arquivos">
            ...
            if (!empty($arquivos)) {
                ...
                echo "<li class='list-group-item'>Nenhum arquivo associado a este professor.</li>";
            }
            ?>
        </ul>
    ...
    <script>
        $(document).ready(function () {
            $('#cadastro-form').submit(function (e) {
                ...
                $.ajax({
                    url: 'servidor/cadastro/adiciona-arquivo-professor.php', // Define o URL de destino
            ...
            $(document).on('click', '.remove-file-btn', function () {
                ...
                var id_professor = <?php echo $id_professor; ?>; // Obtém o ID do professor
                var confirmar = confirm('Tem certeza de que deseja excluir este arquivo?');
                if (confirmar) {
                    $.ajax({
                        url: 'servidor/cadastro/remove-arquivo-professor.php',
                        type: 'POST',
                        data: { arquivo: arquivo, id: id_professor }, // Envia o nome do arquivo e o ID do professor
                        success: function (response) {
                            if (response === true) {
